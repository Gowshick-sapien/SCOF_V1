...
| **D02** | Basic ETL to PostgreSQL, Neo4j, pgvector. | **Enterprise Knowledge Fabric:** PostgreSQL SoR, Materialized Neo4j Topology with bounded query contracts, durable pgvector semantic memory substrate. | **Completed & Verified** | [`D02_knowledge_fabric.md`](./D02_knowledge_fabric.md) |
| **D03** | Simple single-model predictors for demand/inventory. | **Enterprise Demand & Inventory Agents:** Demand Agent (multi-model ensemble + 554 events) and Inventory Agent (multi-echelon stock rules) via MCP. | **Next Phase (D03/D04 Implementation)** | [`D03_demand_inventory_agents_v2.md`](./D03_demand_inventory_agents_v2.md) |
| **D04** | Rule-based delay checks for supplier/transport. | **Enterprise Supplier & Transport Agents:** Supplier Agent (vendor reliability scoring) and Transport Agent (dynamic multi-modal routing) via Neo4j. | **Next Phase (D03/D04 Implementation)** | [`D04_supplier_transport_agents_v2.md`](./D04_supplier_transport_agents_v2.md) |
...
* **Phase A (Foundations — COMPLETED):** D01 (Simulation Foundation) and D02 (Enterprise Knowledge Fabric: PostgreSQL System of Record, Neo4j Materialized Topology, and pgvector Semantic Memory Substrate) fully generated, implemented, and validated across all acceptance gates.
* **Phase B (Specialist Intelligence — NEXT):** Implementing D03 (Enterprise Demand & Inventory Agents) and D04 (Enterprise Supplier & Transport Agents) to reason over the D02 Knowledge Fabric and execute simulations via Cognitive Twin Substrate.
