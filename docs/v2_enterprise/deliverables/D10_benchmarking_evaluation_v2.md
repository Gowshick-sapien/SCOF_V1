...
All benchmark runs strictly enforce the **Tripartite State Isolation Model (ADR ADR 002)** to guarantee zero cross-scenario state contamination and 100% deterministic reproducibility.
...
* **RQ3 (Greedy Bias Mitigation):** Does the CD2F consensus framework (D06) effectively neutralize sub-optimal domain-greedy claims compared to uncoordinated agent panels?
...
* **B3 (Uncoordinated Multi-Agent Panel):** Specialist agents propose actions independently with no CD2F critique or arbitration.
* **SCOF V2 (Full System):** Complete 4-agent specialist panel + LangGraph deliberation + CD2F consensus.
