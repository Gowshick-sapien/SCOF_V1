...
The kernel enforces strict Service Level Agreements (SLAs), manages asynchronous fan-out/fan-in agent rounds, guards against infinite deliberation loops, and guarantees zero mutation of baseline state ([ADR ADR 002](file:///d:/projects/SCOF_V1/SCOF/docs/adr/002_tripartite_state_isolation_for_benchmark_integrity.md)).
...
* **LangGraph Kernel Does NOT Own:** Physics simulation (owned by Twin), consensus arbitration (owned by CD2F), enterprise master data (owned by D2), or physical ERP writes (owned by Execution Adapters).
...
```text
                  ...
                  └──────────────────┬─────────────────────┘
                                     ...
                                     ▼
                  ┌────────────────────────────────────────┐
                  │ 4. Consensus & Decision Hand-Off       │ ──► [D06: CD2F Engine]
                  └──────────────────┬─────────────────────┘
                                     │
                                     ▼
                  ┌────────────────────────────────────────┐
...
4. **Phase 4: Consensus Hand-Off:** The set of claims and critiques is submitted to the CD2F Consensus Engine (D06) for weighted multi-factor arbitration.
